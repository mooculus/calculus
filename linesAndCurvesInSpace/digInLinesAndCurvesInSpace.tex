...
\begin{question}
  ...
  \begin{selectAll}
    ...
    \choice[correct]{$\vector{}+ t\vector{-2,-4,2}$}
    \choice[correct]{$\vector{}+t\vector{1,2,-1}$}

    \choice{$t\vector{2,4,-2}$}%% distractors should have points on line
    \choice{$t\vector{1,2,-1}$}%%
    \choice{$t\vector{7,-14,-7}$}%%
  \end{selectAll}
  \begin{hint}
    Separate each line into its component functions: $x$, $y$, and
    $z$, and see if the equation defining the surface is valid for all
