...
\begin{example}
  ...
  \begin{explanation}
    ...
    \begin{align*}
      ...
      &= (x(\theta,\phi)r\cdot\cos(\theta)\sin(\phi)+a-a)^2+(y(\theta,\phi)-b)^2+(z(\theta,\phi)-c)^2\\
    ...
    Now we need to explain why every point in $S$ is actually drawn by
    ...
    produce the given point $(x,y,z)$. We could do this simply by
    solving for $\theta$ and $\phi$ in terms of $x$, $y$, and $z$
    though we will not do that here. Instead, we will show you
    \textit{how} the sphere is drawn by $\theta$ and $\phi$.
      \begin{image}
        \begin{tikzpicture}
