...
Some values include ``direction'' that is relative to some fixed point.
  ...
  \begin{itemize}
	  \item $v(t)$ is the \dfn{velocity} of an object at time $t$. This represents the ``change in position'' at time $t$.
	  \item $s(t)$ is the \dfn{position} of an object at time $t$. This  gives location with respect to the origin.     
	  	\[  s(t) = s(a)+\int_a^t v(z) \d z. \]
	  \item $s(b) - s(a)$ is the \dfn{displacement}, the distance between the starting and finishing locations.
  \end{itemize}


On the other hand \textit{speed} and \textit{distance} are values without ``direction.''
...
\section{ Net Change in the  amount }
...
For example, we can consider a tank that is being filled with fuel at some rate. Given the rate, we can ask what is the amount of fuel in the 
tank  at a  certain time. Or, a tank that is being emptied at a given rate, or  a culture of bacteria growing in a Petri dish, 
or a  population of a city, etc.
...
\begin{theorem}\hfill
	...
	Then, the \textbf{net change}, $\Delta A$, in the amount $A$ over the time interval $[a,b]$ is given by
	\[ \Delta A = A(b)-A(a)=\int_a^b A'(t)dt. \]
\end{theorem}
\begin {explanation}
	This comes from the Second Theorem of Calculus. 
	It is clear that $\Delta A = A(b)-A(a)$, which gives the change in the amount $A$ over the time interval $[a,b]$. It is the difference between the amount at the end of the time interval and the amount at the beginning of the time interval. 
	
	The right hand side of the equation, the definite integral, gives the ``accumulation of a rate".
	You can think of this definite integral as the limit of Riemann sums, where each Riemann sum is the
	``sum of  the changes of the amount over small intervals of time".
\end {explanation}
...
\item Compute the right Riemann sum of the function $P'$ and the interval $[0,3]$ with $n=3$.
...
Since $n=3$,  $\Delta t=\dfrac{3-0}{3}=1$. Since we are using a right Riemann sum, our sample points will be 
$x_k^* = a + k \Delta t  = 0 + k \cdot 1 = k$.
\begin{align*}
  P(3)-P(0) &= \int_0^3 P'(t) \d t\\
  &\approx\sum_{k=1}^3P'(k)\\
  &=P'(1)+P'(2)+P'(3)\\
...
\begin{definition}\index{average value of a continuous function}
...
\begin{explanation}
From the definition, the average velocity is:
\begin{align*}
...
Like the Intermediate Value Theorem and the Mean Value Theorem for Derivatives, this is an \emph{existential} statement. It tells us that such a number $c$ exists. It does not tell us how to calculate it. If we want to calculate the value of $c$, we will have to rely on our algebra skills.

The Mean Value Theorem for Integrals tells us:
\begin{quote}
	\textbf{The average value of a continuous function is in the range of  the function.}
\end{quote}
\begin{proof}
