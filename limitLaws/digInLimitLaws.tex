...
What if we want to compute the limits

\[
\lim_{x\to \pi} (x^3-\cos{x}),\hspace{0.4in}\lim_{x\to \pi} \frac{\sqrt{2}}{\cos{x}},\hspace{0.2in} or\hspace{0.2in}\lim_{x\to \pi} (\sqrt{2}\cdot x^3\cdot\cos{x})?
\]

Well, we cannot use continuity here, because we don't know if the functions $x^3-\cos{x}$,  $\frac{\sqrt{2}}{\cos{x}}$, and $\sqrt{2}\cdot x^3\cdot\cos{x}$ are continuous at $x=\pi$,
and we have no

 other tools available, since the graphs and tables are not reliable. Obviously, we need more tools to help us with computation of limits.
...
Let's examine how the Limit Laws can be used to help with computation of limits.
%begin{question}
...
\begin{theorem}[Continuity of Polynomial Functions]\index{continuity of polynomial functions}
  ...
  \]
  where $n$ is a positive integer  and each $a_i$ is a real number, are
...
Let
 \[ 
 f(g(x))=\sqrt{\cos{x}}=(\cos{x})^{\frac{1}{2}}
 \]
 where $f(x)=\sqrt{x}=x^{\frac{1}{2}}$, and $g(x)=\cos{x}$.
  Now, continuity of cosine implies that $\lim_{x \to 0}g(x)=\lim_{x \to 0} \cos{x}=\cos{0}=1$.
    Continuity of $x^b$, where $b=1/2$, implies that $f$ is continuous at $1=\lim_{x \to 0}g(x)$.
     Now, the Composition Limit Law implies that
    \[
    \lim_{x \to 0} \sqrt{\cos{x}}= \sqrt{\lim_{x \to 0} \cos{x}}=\sqrt{1}=1
    \]
