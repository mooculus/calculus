...
\section{The idea of parametric equations}
 ...
 Consider an ant crawling along a large flat surface like a floor of a building. Suppose we want to describe the ant's position and the path it takes as it moves. We could introduce an origin as well as a set of $x$ and $y$ axes on the floor. We could approximate the ant as a point and so for each moment in time we could describe the ant's position  by some pair of $x$ and $y$ coordinates. As time passes, the particular path followed by the ant would describe a curve $C$ in the plane. 
...
It's not obvious that this will be a line.  If instead we had used the parameter $s=t^3$, we could trace out the same curve as 
...
\desmos{vj1guvljix}{1024}{500}  \\  



Explore changing the initial velocity in the $x$ and $y$ directions and see how that changes the trajectory of the ball.
...
wthere $t$ runs over the interval $[0, 2\pi)$. Let's we make a table. 
