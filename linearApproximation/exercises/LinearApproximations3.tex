...
The picture below shows the graph of $f$ along with the linear approximation to $f$ at $a=2$.  On this diagram, correctly fill in the labels $\Delta y$ (\verb|\Delta y|), $\d{y}$ (\verb|\d y|), $\d{x}$ (\verb|\d x|), $a$, and $a + \Delta x$ (\verb|a + \Delta x|).
...
\begin{image}
  \begin{tikzpicture}
    \begin{axis}[
	  ...
	  \draw[decorate,decoration={brace,amplitude=3pt,mirror},xshift=-3pt] (axis cs:2,2) -- (axis cs:2,10/7) node[midway,xshift=-15pt]{$\answer{\Delta y}$};
	  \draw[decorate,decoration={brace,amplitude=3pt},xshift=3pt] (axis cs:2.8,2) -- (axis cs:2.8,1.2) node[midway,xshift=15pt]{$\answer{\d y}$};
	  \draw[decorate,decoration={brace,amplitude=3pt},yshift=3pt] (axis cs:2,2) -- (axis cs:2.8,2) node[midway,yshift=15pt]{$\answer{\d x}$};
	  \node at (axis cs:2,0) [below left] {$\answer{a}$};
	  \node at (axis cs:2.8,-0.2) [below right] {$\answer{a + \Delta x}$};
	  \draw (axis cs:2.8,0) -- (axis cs: 3.2,-0.3);
