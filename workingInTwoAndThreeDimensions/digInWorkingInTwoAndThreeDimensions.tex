...
\begin{corollary}
  In general,  a $n$-dimensional ``ball'' of radius
  $r$ centered at the point $(a_1,a_2,a_3,\dots,a_n)$ is given by the inequality
  \[
...
around. Spheres can intersect planes at no points (if they are missing the plane), at one point (if they are
``just'' touching the plane), 
 or  at an infinite number of points (here the intersection is a
circle).
 \begin{image}
      \begin{tikzpicture}
        \draw [penColor2, thick] (-4.5,0) circle [radius=1.5];
          
             
             \draw [penColor6,fill] (-6.5,2) -- (-3,2)--(-2.5,2.5) -- (-6,2.5)--(-6.5,2);
            \draw [penColor6,fill] (-2,1.25) -- (1.5,1.25)--(2,1.75) -- (-1.5,1.75)--(-2,1.25);
               \draw [penColor6,fill] (2.3,-1) -- (6,-1)--(6.5,-0.3) -- (2.8,-0.3)--(2.3,-1);
                \draw [penColor2, thick] (0,0) circle [radius=1.5];
                \draw [penColor2, thick] (4.5,0) circle [radius=1.5];
           \draw[penColor2,very thick] (4.5,-0.65) ellipse (1.35 and .2);
           \draw[dashed,penColor2, thick] (-4.5,0) ellipse (1.5 and .4);
           \draw[ thick,penColor2] (-6,0) arc (180:360:1.5 and .4);
            \draw[dashed,penColor2, thick] (0,0) ellipse (1.5 and .4);
             \draw[ thick,penColor2] (-1.5,0) arc (180:360:1.5 and .4);
             \draw[dashed,penColor2, thick] (4.5,0) ellipse (1.5 and .4);
              \draw[ thick,penColor2] (3,0) arc (180:360:1.5 and .4);
                \draw[penColor,very thick] (4.5,-0.65) ellipse (1.35 and .2);
                 \draw [penColor, fill] (0,1.5) circle [radius=.1];
      \end{tikzpicture}
    \end{image}
...
\begin{example}
  ...
  \begin{explanation}
    ...
    the $(y,z)$-plane is the plane $\answer[given]{x}=0$.
    ...
    x=0
    \]
  By plugging  $x=0$ into the first equation, we obtain that 
    \[
   (y+2)^2 + (z-1)^2 =0
    \]
   So, the sum of the  two squares  on the left is equal to $0$. This implies that both terms on the left are equal to $0$.
Therefore, the system has a unique solution, namely
     %Since the
...
\begin{example}
  ...
  \begin{explanation}
    ...
    the $(x,y)$-plane is the plane $\answer[given]{z}=0$. 
    %Since the
   ...
   % $\answer[given]{\sqrt{8}}$, there are an infinite number of
    ...
    So, all the points on the intersection satisfy the system of equations
     \[
    (x-3)^2 + (y-1)^2 + (z+2)^2 = 8
    \]
     \[
    z=0
    \]
     By plugging  $z=0$ into the first equation, we obtain that 
      \[
    (x-3)^2 + (y-1)^2  = 4
    \]
       Hence, the intersection consists of infinitely many points and they form a circle of radius $r=\answer[given]{2}$. The circle of intersection is centered at    $(\answer[given]{3},\answer[given]{1},\answer[given]{0})$, in
...
\begin{example}
  ...
  \begin{explanation}
    ...
    We could reach the same conclusion by setting up a system of equations
     \[
    (x+1)^2 + (y+3)^2 + (z-2)^2 = 6
    \]
     \[
  y=0
    \]
    By plugging $y=0$ into the first equation, we get that
     \[
    (x+1)^2 +  (z-2)^2 = -3
    \]
    which implies that the system has no solutions.
  \end{explanation}
\end{example}
