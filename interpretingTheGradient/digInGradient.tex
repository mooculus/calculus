...
\begin{question}
  Consider a differentiable function $H:\R^2\to\R$ and the line
  $\vecl(t) = \vector{1-2t,3+t}$. Suppose that
  \[
...
\begin{example}
  Let $F:\R^2\to\R$ be a differentiable function described by the
    ...
    directly toward $F(2,6)= \answer[given]{16}$, the greatest initial increase from
    $(3,5)$.
    \end{explanation}
\end{example}

\begin{question}
  Here is a plot of an elliptic paraboloid $G(x,y) = x^2 + y^2$ along with several vectors:
  \begin{image}
    \begin{tikzpicture}
      \begin{axis}%
	  ...
	  zlabel={\scriptsize $z$},colormap/cool
        ...
        \addplot3[->] coordinates {(2,-2,8) (3,-3,8)};
      \end{axis}
    \end{tikzpicture}
  \end{image}
  HERE
...
EXAMPLE OF LEVEL CURVES WITH GRADIENT VECTOR COMPARED TO MAGNITUDE OF SOME DIRECTIONAL DERIVATIVE




SOMETHING ABOUT THE MAGNITUDE OF GRADIENT????
