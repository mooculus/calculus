...
\begin{enumerate}
\item A point $(x,f(x))$ is a \dfn{local maximum} if there is an interval $a<x<b$ with $f(x)\ge f(z)$ for
  every $z$ in $(a,b)$.
\item A point $(x,f(x))$ is a \dfn{local minimum} if
...
minimum are points at which the derivative is zero, consider the plots
of $f(x) = x^3-4x^2+3x$ and $f'(x) = 3x^2-8x+3$,
\begin{image}
\begin{tikzpicture}
	\begin{axis}[
            domain=-3:3,
...
\begin{definition}\index{critical point}
  ...
  \]
  is called a \dfn{critical point} for $f$.
\end{definition}
