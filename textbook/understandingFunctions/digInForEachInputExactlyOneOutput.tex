...
tools for talking about these relationships.  
...
from the \dfn{domain}\index{domain} and we call the set a function is
mapping to the \dfn{range}\index{range}.
...
by the plot shown in the plot below:
...
integer less than or equal to $x$. For your viewing pleasure, here is
a plot of the greatest integer function:
...
\end{image}
Some might be confused because here we have multiple
...
f(x) = \sqrt{x}.
\]
The domain is $\{x: x\ge 0\}$ written in set notation or $[0,\infty )$
  in interval notation.  In calculus, we'll often use interval
  notation.  The range is $(-\infty,\infty)$.  Here is a plot of $f$:
...
\begin{example}
  ...
  \begin{explanation}
    ...
    We see that $\sqrt{x^2}\ne x$ as $f(-2)=2$ and $f(-1)=1$.
    Instead, we see that $\sqrt{x^2} = |x|$.  The domain of
    $f(x)=\sqrt{x^2}$is $(-\infty,\infty)$ and the range is
    $[0,\infty)$.  See figure for plot.
...
\begin{example}
  ...
  the same function?
  \begin{explanation}
    What if we compare plots? Here we see a plot of
    $f$:
    \begin{image}
      \begin{tikzpicture}
	\begin{axis}[
...
\end{image}
On the other hand, here is a plot of $g$:
\begin{image}
\begin{tikzpicture}
	\begin{axis}[
            domain=-2:4,
...
%% \label{plot:point undfed fxn}
\end{image}
What if we compare domain?  We see a hole in $f$ at $x=2$.  This is
where $f$ is undefined.  On the other hand, there is no hole in
$g$ and the domain is $(-\infty, \infty)$.  Thus, these are not
the same function.

What we can say is that $f(x)=g(x)$, or
\[
f(x)=x-2\qquad\text{when}\qquad x \ne 2.
\]
From this we see that it is critical to think about the domain and
range of a function.
  \end{explanation}
\end{example}
