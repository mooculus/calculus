...
\begin{question}
  ...
  \begin{image}
    ...
    \end{tikzpicture}
  \end{image}
  Is the function $f$ invertible at $x=1$? (That is, does $f^{-1}(1)$ exist?)
  	\begin{hint}
		Is there exactly one value of $b$ with $f(b)=1$?
  	\end{hint}
  \begin{prompt}
  \begin{multipleChoice}
    \choice[correct]{yes}
...
\begin{example}
  ...
  \begin{explanation}
    ...
    interval $[0,\infty)$. Hence we can find an inverse of $f(x)=x^2$ on
	  ...
	  \addplot [very thick, penColor, smooth,domain=0:2] {x^2};
	    \addplot [dashed, very thick, penColor, smooth,domain=-2:0] {x^2};
          \addplot [ very thick, penColor2, smooth, samples=100,domain=0:2] {sqrt(x)};
          \addplot [dashed, textColor] {x};
           \node at (axis cs:0.8,.45) [penColor] {$f$};
