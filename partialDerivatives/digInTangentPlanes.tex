...
\begin{image}
  \begin{tikzpicture}
	...
	zlabel={\scriptsize $z$},
      ...
      \addplot3[domain=0:4,,y domain=0:4,surf,opacity=.5,samples=15,samples y=15,very thin,z buffer=sort] {-.2*(x-1)^2-.05*y^2+2};
      \addplot3[domain=0:4,,y domain=0:4,surf,red,opacity=.5,samples=15,samples y=15,very thin,z buffer=sort] {2.8-.4*x-.2*y};
      \filldraw [penColor] (axis cs:2,2,1.6) circle (2.5pt);
      \filldraw [penColor2] (axis cs:3,3,1) circle (2.5pt); 
    \end{axis}
  \end{tikzpicture}
\end{image}
