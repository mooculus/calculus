...
\choice[correct]{Both students are wrong.}
\end{multipleChoice}
...
In their argument, student $1$ attempted to use a path along which $x=0$.  Can $\vector{x,y}$ approach $\vector{3,1}$ along this path? \wordChoice{\choice{Yes}\choice[correct]{No}}

Student $1$ also attempted to use a path along which $y=0$.  Can $\vector{x,y}$ approach $\vector{3,1}$ along this path? \wordChoice{\choice{Yes}\choice[correct]{No}}

\item Now let's look at Student $2$'s argument.  In the argument, Student $2$ tried to analyze the function the most general straight line path.  However, note that $y=mx$ is the most general straight line path through $(0,0)$; there's only one value for $m$ for which $y=mx$ passes through $(3,1)$, and that is $m= \answer{\frac{1}{3}}$.
