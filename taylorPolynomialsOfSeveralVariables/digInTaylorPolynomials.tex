...
\begin{definition}
  ...
  \]
\end{definition}
As you can see, it is more complex than the formula for a single
variable. Good news everyone: \textbf{In this class, we will only compute
the degree two polynomial for functions of two variables.} In this case
$P_2$ is:
\begin{align*}
P_2(\vec{x})=F(\vec{c})
...
\begin{example}
  ...
  \begin{explanation}
    ...
    \begin{align*}
      P_2(x,y) &= (1/2)\left(\answer[given]{2}\cdot (y-0)^2 + 2 \left(\answer[given]{-1}\right)(x-(-1))(y-0) \right)\\
...
\begin{example}
  ...
  \begin{explanation}
    ...
    We see our polynomial is
    \begin{align*}
      P_2(x,y) &= 6 + (1/2)\left(\answer[given]{-6}\cdot (x-(-1))^2 +  \left(\answer[given]{-2}\right)(y-2)^2 \right)\\
...
This final section is for the interested student, and is not required
for this course.

Recall that if $F:\R^n\to\R$ is a function whose first $d$ derivatives
exist at $\vec{x}=\vec{c}$.  The \dfn{Taylor polynomial} of degree $d$
of $F$ centered at $\vec{x}=\vec{c}$ is
\[
P_d(\vec{x}) = \sum_{k=0}^d \eval{\eval{\frac{(\vec{a}\dotp \grad)^k F(\vec{x})}{k!}}_{\vec{x}=\vec{c}}}_{\vec{a}=\vec{x}-\vec{c}}
\]
...
Whew. That was a lot of work. However, as we have said before, all you
need to know right now, is how to compute the degree two Taylor
polynomial for functions of two variables.
