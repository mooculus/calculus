...
\begin{definition}
Let $F: \R^n\to \R$ and let $\vec{x}=\vector{x_1, x_2, ..., x_n}$ and $\vec{a} = \vector{a_1, a_2, ..., a_n}$ be vectors in the interior of the domain of $F$.  We say $F$ is  \dfn{differentiable} at a vector $\vec{a}$ if there exists $T_{\vec{a}}(\vec{x})=c_0+\sum_{i=1}^n c_i (x_i-a_i)$ for some real constants $c_i$ such that 
\[
...
yet some partials may not be continuous. It is also possible for all the partial derivatives to exist at a point and the function still fail to be continuous, let alone differentiable, at that point.  Such strange behavior of
