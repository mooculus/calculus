...
are interested in finding specific angles.  For instance, you may want to find some angle $\theta$ such that
\[
\cos(\theta) = .7
\]
...
\begin{definition}\hfil
  \begin{itemize}
    \item $\arccos(x) = \theta$ exactly when $\cos(\theta) = x$ and
      ...
      1$.
    \item $\arcsin(x) = \theta$ exactly when $\sin(\theta) = x$ and
    ...
    \item $\arccot(x) = \theta$ exactly when $\cot(\theta) = x$ and
      $0< \theta< \pi$. The domain of $\arccot(x)$ is
      $-\infty<x<\infty$.
    \item $\arcsec(x) = \theta$ exactly when $\sec(\theta) = x$ and
      $0\le \theta\le \pi$ with $x\ne \pi/2$. The domain of
      $\arcsec(x)$ is all $x$ with absolute value greater than $1$.
    \item $\arccsc(x) = \theta$ exactly when $\csc(\theta) = x$ and
      $-\frac{\pi}{2}\le \theta\le \frac{\pi}{2}$ with $x\ne 0$. The
