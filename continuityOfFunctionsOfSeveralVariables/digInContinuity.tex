...
\begin{definition}
  ...
  \end{itemize}
  \item Given a set $S$, we denote the \dfn{boundry} of $S$ by
    $\partial S$.
\end{definition}
...
\begin{example}
  ...
  \begin{explanation}
    Since $\partial D$ is the boundry of a closed disk in $\R^2$, $\partial D$ is \wordChoice{
      \choice{a disk}
      \choice[correct]{a circle}
      \choice{a ball}
      \choice{a line}}. Since $\partial\partial D$ is the boundry of the boundry, and a circle has no boundry, $\partial\partial D$ is \wordChoice{
