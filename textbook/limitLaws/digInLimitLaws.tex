...
For the purpose of this course, the young mathematician must be able
to recognize when limit laws apply. We end this section with a
question:
...
\begin{question}
  ...
  \begin{multipleResponse}
    ...
    \choice{$\lim_{x\to 0} x\cos(1/x)$}
    \choice{$\lim_{x\to 0} \cot(\sin(x^3))$}
    \choice[correct]{$\lim_{x\to 0}\sec^2(e^x-1)$}
    \choice{$\lim_{x\to 1}{(x-1)\cdot \csc(\ln(x))}$}
    \choice{$\lim_{x\to 0} x\ln x$}
    \choice[correct]{$\lim_{x\to 0}\frac{2^x-1}{3^{x-1}}$}
    \choice{$\lim_{x\to 0}(1+x)^{1/x}$}
