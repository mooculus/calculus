...
Compute $\dd[x]{y}$.
\begin{explanation}
First, notice that the problem asks for $\dd[x]{y}$, \dfn{not} $\dd[y]{x}$.  
So we are considering $x$ as a function of $y$.....the variables have changed places!
Not to worry, everything is exactly the same.
We apply the $\dd[]{y}$ operator to both sides of the equation to get
\[
\dd[]{y} \left( \cos(xy) - \frac{y}{x} \right) = \dd[]{y} (4x^2 y^3)
\]
which gives us
\[
-\answer[given]{\sin(xy)} \left(y \dd[x]{y} + x \right) - \frac{\answer[given]{x - y \dd[x]{y}}}{x^2} = \answer[given]{8xy^3 \dd[x]{y}} + 12 x^2 y^2 .
\]
Distributing and multiplying across by $x^2$ yields
\[
-x^2 y \sin(xy) \dd[x]{y} - x^3 \sin(xy) - x + y \dd[x]{y} = 8x^3y^3 \dd[x]{y} + 12x^4y^2.
\]
Grouping terms, factoring, and dividing finally gives us
\begin{align*}
-x^2 y \sin(xy) \dd[x]{y} + y \dd[x]{y} - 8x^3y^3 \dd[x]{y} &= x^3 \sin(xy) + x + 12x^4 y^2  \\
