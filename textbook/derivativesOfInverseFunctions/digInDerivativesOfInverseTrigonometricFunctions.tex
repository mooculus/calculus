...
\begin{explanation} 
  ...
  Recall
\[
\arcsin(x) = \theta\qquad \text{exactly when} \qquad\sin(\theta) = x\qquad\text{and}\qquad -\frac{\pi}{2}\le \theta\le \frac{\pi}{2}.
\]
...
\end{align*}
At this point, we would like $\theta'$ written in terms of $x$. Since
...
\begin{tikzpicture}
	\begin{axis}[
          ...
          \addplot [black!30!white, dashed, smooth, domain=(90:270)] ({cos(x)},{sin(x)}); %% unit circle
...
\[
\ddx \theta = \ddx \arcsin(x) = \frac{1}{\sqrt{1-x^2}}.
\]
\end{explanation}
\end{theorem}
...
\begin{explanation} 
  ...
  Recall
\[
\arctan(x) = \theta\qquad \text{exactly when} \qquad\tan(\theta) = x\qquad\text{and}\qquad -\frac{\pi}{2}< \theta< \frac{\pi}{2}.
\]
Implicitly differentiating with respect $x$ we see
\begin{align*}
\tan(\theta) &= x\\
...
\begin{tikzpicture}
	\begin{axis}[
          ...
          \addplot [black!20!white, dashed, smooth, domain=(90:270)] ({cos(x)},{sin(x)}); %% unit circle
          ...
          %\addplot [very thick, penColor, smooth, domain=(0:40)] ({cos(x)},{sin(x)}); %% sector
          \node at (axis cs:.15,.07) [anchor=west] {$\theta$};
          \node at (axis cs:.84,.322)[anchor=west] {$\frac{x}{\sqrt{x^2+1}}$};
          \node at (axis cs:.383,0) [anchor=north] {$\frac{1}{\sqrt{x^2+1}}$};
          \node at (axis cs:.38,.32) [anchor=south] {$1$};
        \end{axis}
\end{tikzpicture}
\end{image}
...
To be completely explicit, 
\[
\ddx \theta = \ddx \arctan(x) = \frac{1}{1+x^2}.
\]
\end{explanation}
\end{theorem}
...
\begin{tikzpicture}
	\begin{axis}[
          ...
          \addplot [black!20!white, dashed, smooth, domain=(180:360)] ({cos(x)},{sin(x)}); %% unit circle
