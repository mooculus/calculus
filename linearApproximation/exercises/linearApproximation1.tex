...
Since $f''(x)<0$ on $(9, 9.5)$, the graph of $f$ is concave down on that interval. %Therefore, the tangent line lies above the graph of $f$, except at $x=9$, where $f(9)=L(9)$.
[Note: We know $f''(x) < 0$ on the entire interval $(0, \infty)$, but only the portion on $(9, 9.5)$ matters for determining if we have an underestimate/overestimate.]

This means that $L(9.5)\ge f(9.5)$.
\end{hint}

This is an
 \wordChoice{\choice{underestimate}\choice[correct]{overestimate}}
because the graph of $f$ is \wordChoice{\choice{Concave Up}\choice[correct]{Concave Down}}
on the interval $\left( \answer{9}, \answer{9.5} \right)$.
\end{prompt}
