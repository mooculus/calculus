...
\item[Riley] That sounds like a cool idea.  One example would be, since we know the graph of $f(x) = \sqrt{1-x^2}$ is a semicircle, we get an ellipse defined on $[-2,2]$ just by stretching the graph of $f$ by a factor of $2$ horizontally.  The equation of this ellipse would be $g(x) = $ \answer{sqrt(1-(x/2)^2)}
\item[Devyn] Exactly!  So since we know that $\int_{-1}^1 \sqrt{1-x^2} dx = $\answer{pi/2} geometrically...
