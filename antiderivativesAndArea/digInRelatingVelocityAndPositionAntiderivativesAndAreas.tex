...
From the graph above, we see that the assumption of constant velocity is not quite accurate. The velocity is changing, so we should allow our velocity to change as well.
Instead of assuming the velocity is constant over the entire four-second interval, 
we will keep approximating the velocity with a constant over smaller and smaller intervals of time.
...
\begin{theorem}\label{thm:Disp}
	Suppose $v$ is the (continuous) velocity function of an object traveling along a straight line during the interval $[a, b]$, which has position function $s$. 
	The displacement of the object can be found in two ways:
	\begin{itemize}
		\item As $\Delta s = s(b)-s(a)$ and
		\item As a definite integral $\displaystyle \int_a^b v(t) \d t$.
	\end{itemize}
	In particular, the definite integral of a velocity function can be calculated by finding
	the distance function:
	\[ \int_a^b v(t) \d t = s(b) - s(a) \]
\end{theorem}

This means we are able to evaluate the definite integral \textbf{of velocity functions} by calculating indefinite integrals.
...
\begin{example}
	...
	\begin{explanation}
		...
		\begin{align*}
			s(t) &= \int v(t) \d t\\
				&= \int \left( 3\sin(t) + e^t\right) \d t \\
				&= 3\cos(t) + e^t + C
		\end{align*}
		...
		\begin{align*}
			s(\pi)-s(0) &= \left(3\cos(\pi)+e^\pi + C\right) - \left(3\cos(0)-e^0+C \right) \\
				&= (-3+e^\pi+ C) - (3-1+C)\\
				&= \answer[given]{-5+e^\pi}
		\end{align*}
		Thus
		\[ \int_0^\pi v(t) \d t = -5+e^\pi \unit{meters}\]
...
We now know that we can find the definite integral \textbf{of velocity functions} by finding antiderivatives. In the next few sections we will see if this can be extended to definite integrals of other functions.
