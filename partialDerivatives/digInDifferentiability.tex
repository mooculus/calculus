...
In $\R^2$, we can interpret this definition as saying that a function $F$ is differentiable at a vector$\vec{a}$ if there is a tangent plane $T_{\vec{a}}(\vec{x})$ at that point such that $F$ approaches its  tangent plane $T_{\vec{a}}(\vec{x})$ faster than $\vec{x}$ approaches $\vec{a}$.  That is, if one ``zooms in'' on the graph of $F$ at $(\vec{a}, F(\vec{a}))$ sufficiently, it  looks more and more like the tangent plane.
