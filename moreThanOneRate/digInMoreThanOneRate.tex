   ...
   We \textbf{identify} the given rate, $\frac{dP}{dt}=4$ m/s, and the unknown rate, $\Bigl[\frac{dA}{dt}\Bigr]_{r=3}$, the rate to be determined. 
    ...
    \begin{image}
      \begin{tikzpicture}
        ...
        \node [penColor,left] at (5,1.02) {$\dfrac{d}{dt}P(t) = 4$ m/s};
        \node [penColor,left] at (4.65,-0.2) {$\Bigl[\frac{dA}{dt}\Bigr]_{r=3} = $ ? };
    ...
    Next, we \textbf{draw a picture}.
     \begin{image}
      ...
      \tkzMarkAngle[size=0.6cm,thin](B,C,A)
        ...
        \node [left] at (a) {$a = 3 m$};
           \node [below] at (b) {$b$};
              \node [penColor] at (1.16,0.22){$\theta$};
      \node [penColor] at (2.4,2.42){$\frac{db}{dt} = 2$};
        \node [penColor,left] at (3.9,1.25) {$\Bigl[\frac{d\theta}{dt}\Bigr]_{b=3} = $ ? };
      \end{tikzpicture}
    \end{image}
