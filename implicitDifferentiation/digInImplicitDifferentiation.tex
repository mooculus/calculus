...
Nevertheless, we are often faced with a problem of computing derivatives of such functions. Therefore, we need a new method that will enable us to compute derivatives of implicitly defined functions.
...
\end{align*}
Remark: You may surprised that the derivative $\dd[y]{x} $ is expressed in terms of both variables, $x$ and $y$. This should not come as a surprise. If you think about it, the values  of the derivative and of the function are not determined by the value of $x$ only. Check what happens if $x=\frac{\sqrt{2}}{2}$.
