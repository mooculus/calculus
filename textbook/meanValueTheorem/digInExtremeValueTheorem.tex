...
\begin{enumerate}
\item A function $f$ has an \dfn{absolute maximum} at $x=a$ in an
  interval if $f(a)\ge f(x)$ for every $x$ and $a$ in that interval.
\item A function $f$ has an \dfn{absolute minimum} at $x=a$ in an interval if
  $f(a)\le f(x)$ for every $x$ in that interval.
\end{enumerate} 
An \dfn{absolute extremum}\index{extremum!absolute} is either an
