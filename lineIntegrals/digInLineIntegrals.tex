...
\begin{example}
  ...
  \begin{image}
    \begin{tikzpicture}[x=1cm,y=.7cm]
      ...
      \node at (4,1.5) {$2\veci+\vecj$};
      \node at (6,1.5) {$\veci-\vecj$};
      \node at (8,1.5) {$\veci+\vecj$};
      ...
      \node at (4,3.5) {$-\veci+\vecj$};
      \node at (6,3.5) {$\veci-\vecj$};
      \node at (8,3.5) {$2\veci+\vecj$};
  ...
  \begin{explanation}
    ...
    \[
    \vector{1,-1}\dotp \vector{-2/3,-2/3}+\vector{1,0}\dotp \vector{-2/3,-2/3}\vector{1,-1}\dotp \vector{-2/3,-2/3}
    \]
    Thus we see that:
    \[
...
\begin{example}
  ...
  \begin{explanation}
    ...
    above.  Write with me
    \begin{align*}
    \answer[given]{0}&\cdot (-2) + \answer[given]{2} \cdot (-1) && \text{(for the first field vector)}\\ 
    &+ (-1) \cdot (-2)  + 2 \cdot (-1) && \text{(for the second field vector)}\\
    &+ (-2)\cdot (-2) + 1 \cdot (-1) && \text{(for the third field vector)}\\
    &+ (-3)\cdot (-2) + 0\cdot (-1) && \text{(for the fourth field vector)}\\
    &+ (-1)\cdot (-2) + (-1)\cdot (-1) && \text{(for the fifth field vector)}\\ 
    &+ (0)\cdot (-2) + (-1)\cdot (-1)&& \text{(for the sixth field vector)}
    \end{align*}
...
\begin{example}
  ...
  \begin{explanation}
    ...
    direction. While there are lots of ways to do this, we'll break it
    into four line segments each parameterized as $t$ runs from $0$ to
    $1$ (though any parameterization would work).
    \begin{image}
    \begin{tikzpicture}
      \begin{axis}%
        [
...
end-points to compute the line integral. Because of this, some people call gradient fields \dfn{path independent fields}. Let's see an example.
...
\begin{definition}
  A vector field $\vec{F}:\R^n\to\R^n$ is a \dfn{conservative field} if
  \[
...
field a \dfn{conservative field} is just another name for a
\dfn{gradient field}.
