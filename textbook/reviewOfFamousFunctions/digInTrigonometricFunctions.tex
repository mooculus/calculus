...
above, this make absolutely no difference when computing sine and
...
\end{center}
we obtain all of the trigonometric functions.
...
%% \end{figure*}
\end{image}
If we restrict $\cos(\theta)$ to this interval, then this restricted
function is one-to-one and hence has an inverse.

\begin{question}
  What arc on the unit circle corresponds to the restricted domain
  described above of $\cos(\theta)$?
  \begin{multipleChoice}
    \choice{\begin{tikzpicture}[framed,scale=.3,baseline=7ex]
	\begin{axis}[
            xmin=-1.1,xmax=1.1,ymin=-1.1,ymax=1.1,
            axis lines=center,
            width=4in,
        ...
        \end{axis}
    \end{tikzpicture}}
    \choice[correct]{\begin{tikzpicture}[framed,scale=.3,baseline=7ex]
	\begin{axis}[
            xmin=-1.1,xmax=1.1,ymin=-1.1,ymax=1.1,
            axis lines=center,
            width=4in,
...
In a similar fashion, we need to restrict sine to be able to take an
inverse. The function $\sin(\theta)$ takes on all values between $-1$
and $1$ exactly once on the interval $[-\pi/2,\pi/2]$.
...
If we restrict $\sin(\theta)$ to this interval, then this restricted
...
Here we see a plot of $\arccos(x)$, the inverse function of
...
& 
\begin{minipage}{2.5in}
Here we see a plot of $\arcsin(x)$, the inverse function of
...
\]
are called ``arc'' because they literally take the length of an
``arc'' of the unit circle and return the corresponding angle.
...
an inverse function, arctangent. Here we see a plot of $\arctan(y)$,
the inverse function of $\tan(\theta)$ when it is restricted to the
interval $(-\pi/2,\pi/2)$.
...
We are now please to present the so-called ``inverse'' trigonometric
functions.
...
\begin{tikzpicture}
	\begin{axis}[
            xmin=-6.75,xmax=6.75,ymin=-1.5,ymax=3.375,
            axis lines=center,
            xtick={-6.28, -4.71, -3.14, -1.57, 0, 1.57, 3.142, 4.71, 6.28},
            xticklabels={$-2\pi$,$-3\pi/2$,$-\pi$, $-\pi/2$, $0$, $\pi/2$, $\pi$, $3\pi/2$, $2\pi$},
            ytick={-1,1},
          ...
          \addplot[color=penColor5,fill=penColor5,only marks,mark=*] coordinates{(1,0)};  %% closed hole          
          \addplot[color=penColor5,fill=penColor5,only marks,mark=*] coordinates{(-1,pi)}; 
          
        \end{axis}
\end{tikzpicture}
\end{image}
\[
\arccos(x) = \theta\qquad \iff\qquad\cos(\theta) = x\text{ and } 0\le \theta\le \pi
\]
The domain of $\arccos(x)$ is $-\le x\le 1$.
...
\begin{tikzpicture}
	\begin{axis}[
            xmin=-6.75,xmax=6.75,ymin=-2,ymax=2,
            axis lines=center,
            xtick={-6.28, -4.71, -3.14, -1.57, 0, 1.57, 3.142, 4.71, 6.28},
            xticklabels={$-2\pi$,$-3\pi/2$,$-\pi$, $-\pi/2$, $0$, $\pi/2$, $\pi$, $3\pi/2$, $2\pi$},
            ytick={-1,1},
...
\end{image}
\[
\arcsin(x) = \theta\qquad \iff\qquad\sin(\theta) = x\text{ and } -\frac{\pi}{2}\le \theta\le \frac{\pi}{2}
\]
The domain of $\arcsin(x)$ is $-1\le x\le 1$. 

\paragraph{Arctangent}
   

     \[
     \arctan(x) = \theta\qquad \iff\quad\tan(\theta) = x\text{ and } -\frac{\pi}{2}< \theta< \frac{\pi}{2}
     \]
     The domain of $\arctan(x)$ is $-\infty<x<\infty$.
     

\[
     \arccot(x) = \theta\qquad \iff\quad\cot(\theta) = x\text{ and } 0< \theta< \pi
\]
The domain of $\arccot(x)$ is $-\infty<x<\infty$. 
    
   \begin{align*}%% Domain of \{x:|x|\ge 1\}
    \arcsec(x) = \theta\qquad &\iff\qquad\sec(\theta) = x\text{ and } 0\le \theta\le \pi\text{ with } x\ne \pi/2\\
    \arccsc(x) = \theta\qquad &\iff\qquad\csc(\theta) = x\text{ and } -\frac{\pi}{2}\le \theta\le \frac{\pi}{2}\text{ with } x\ne 0
  \end{align*}

\section{The Power of the Pythagoriean idenity}
