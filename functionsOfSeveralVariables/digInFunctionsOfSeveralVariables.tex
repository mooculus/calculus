...
\begin{definition}
  A \dfn{function of several variables} with \dfn{domain} $D$ ($D$ is
  a subset of $\R^n$) of $\R^n$ is a relation
  \begin{align*}
    F: D &\to \R\\
    \vec{x} &\mapsto y\\
    \vector{x_1,x_2,\dots,x_n} &\mapsto y
  \end{align*}
  that assigns every ordered tuple in $D$ a unique real number in
  $\R$. The set of all outputs of $F$ is the \dfn{range} (a subset of
  $\R$).
\end{definition}
