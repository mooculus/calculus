...
From the two sequences we've just considered, the method of ``finding a pattern'' is \textbf{not enough} when dealing with sequences unless you understand exactly how the sequence was produced.   However, having to define each term explicitly is quite cumbersome.  In general, we want to define a sequence by specifying a rule that will allow us to write down any term that we want.  There are two common ways to do this:

\section{Two common methods of representing sequences}

\begin{definition}
We say that a sequence $\{a_n\}_{n=1}$ is described by an \dfn{explicit formula} the term $a_n$ is expressed as a function of $n$ only.
\end{definition}
...
\begin{warning}
  ...
  generating the sequence.  For instance, the sequence:
  
  \[
  -1,1,-1,\dots
  \]
  is described by the rule $a_n = (-1)^n$.  The sequence $\{a_n\}$ is the actual list of numbers above.  The expression $a_n = (-1)^n$ is a term of the sequence, \emph{not} the sequence itself. 
\end{warning}
...
\begin{definition}
We say that a sequence $\{a_n\}_{n=1}$ is described by a \dfn{recursive formula} if the term $a_n$ is expressed as a function of $n$ and at least one of the previous terms in the sequence.
\end{definition}
