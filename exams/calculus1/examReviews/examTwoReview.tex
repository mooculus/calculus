...
The average velocity over the interval $[1,3]$ is $v_{av}=\answer{21}$

(c) Select all reasonable approximations for the average velocity over the interval $[1,3]$:
\begin{selectAll}
\choice[correct]{32}
\choice{10}
\choice[correct]{26}
...
(b) Let $f(x)=\frac{1}{x-4}$. Evaluate $f$ in the equation in (a) to fill in the blanks:
 $\lim_{\answer[format=string]{h}\to\answer{0}}\frac{\frac{1}{\answer{(x+h)-4}}-\frac{1}{\answer{x-4}}}{\answer{h}}$

(c) Using the (limit) defintion of the derivative in (b), compute $f'(x)=\answer{-(x-4)^{-2}}$
\end{exercise}
...
(a) $y=\left(5\sin(x)\right)\arctan\left(\frac{x}{x+1}\right)$
\begin{multipleChoice}
...
\begin{multipleChoice}
\choice[correct]{$\sin(x)^{ax}(ax\cot(x)+a\log(\sin(x)))$}
\choice{$ax\sin(x)^{ax-1}\cos{x}$}
\choice{$a\sin(x)^{ax}\log(\sin(x))$}
\end{multipleChoice}

(ii) Find the following values or state the value is undefined (DNE):

(a) $\sin\left(\frac{17\pi}{6}\right)=\frac{\answer{1}}{\answer{2}}$
...
(i) At $x=1$, $\ddx\left[g(x)\right]$=\begin{multipleChoice}
\choice{$\frac{13}{3}$}
\choice{$\frac{4}{3}$}
\choice{12}
\choice{4}
...
(c) Find the x-coordiante of all points in the interval $(0,5)$ where $f$ has a local minimum: $x=\answer{1}$

(d) Find the x-coordinate of all points in the interval $(0,5)$ where $f$ has a local maximum: $x=\answer{2}$

(e) Find all critical points in the interval $(0,5)$: $x=\answer{1},\answer{2}$
\end{exercise}
...
(d) What are the velocity and acceleration at that time? $v=\answer{0}$ $\frac{\text{ft}}{\text{min}}$, $a=\answer{-80} \frac{\text{ft}^2}{\text{min}}$

(e) At what (positive) time is the object at the origin? $t=\answer{6}$ min

(f) What are the velocity and acceleartion at that time? $v=\answer{-320}$ $\frac{\text{ft}}{\text{min}}$, $a=\answer{-80}$ $\frac{\text{ft}^2}{\text{min}}$
...
(a) At $x=-3$, $\ddx\left[f^{-1}(x)\right]=\answer{-2}$

(b) List all critical points of $f$: $\answer{-8},\answer{3}$

(c) List the x-coordinates of all local minima in $(-8.8)$ of $f$, or say `none': $x=\answer{3}$

(d) List the x-coordinates of all local maxima in $(-8,8)$ of $f$, or say `none': $x=\answer[format=string]{none}$
