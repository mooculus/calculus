...
\begin{example}
Suppose now that we have two functions, $y=4x^3+17$ and $y=3x^2$ and suppose we want to find the area between the two curves over the interval $-1 \leq x \leq 1$.  The area is shown below.
...
We cannot determine the exact area of the slice, but just as before, we can approximate that each slice by a rectangle whose heights are determined by the value of the function $y = f(x)$ at some $x$-value on its base.  Here, we choose to use the lefthand endpoints.
...
\begin{itemize}
\item If the top or bottom curve of the region depends on where you draw the slice, you'll need \wordChoice{\choice{one}\choice[correct]{more than one}} integral with respect to $x$ to find the area.
\item If the left or right curve of the region depends on where you draw the slice, you'll need \wordChoice{\choice{one}\choice[correct]{more than one}} integral with respect to $y$ to find the area.
\end{itemize}
