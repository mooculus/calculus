...
all is lost. We will have an analogue of the sum rule for derivatives.% and the constant factor rule.
\begin{question}
  Find H, an antiderivative of the function $h$, where $h(x)=\cos{x}+\frac{1}{x}$.
  \begin{multipleChoice}
  ...
  \begin{feedback}
    Differentiate each choice for $H(x)$.
     ...
     In part (c) we get $H'(x)=\cos{x}+\frac{1}{x}$.
  \end{feedback}
\end{question}
It is easy to recognize an antiderivative: we just have to differentiate it, and check wether $H'(x)=h(x)$, for all $x$ in the domain of $h$.
But, can we on our own find an antiderivative of a sum of two functions?
\begin{question}
Let $f$, $g$, $F$ and $G$ be functions such that 

$F'(x)=f(x)$ and $G'(x)=g(x)$ on some interval $I$.

  Find H, an antiderivative of the function $h$, where $h(x)=f(x)+g(x)$.
  \begin{multipleChoice}
    \choice{$H(x)=f(x)+g(x)$}
    \choice{$H(x)=F(x)-g'(x)$}
    \choice[correct]{$H(x)=F(x)+G(x)$}
  \end{multipleChoice}
  \begin{feedback}
    Differentiate each choice for $H(x)$. In part (c) we get
    
     $H'(x)=\left(F(x)+G(x)\right)'=F'(x)+G'(x)=f(x)+g(x)=h(x)$.
     
    Therefore, $F+G$ is an antiderivative of $f+g$. 
  \end{feedback}
 To summarize: an antiderivative of a sum is the sum of antiderivatives. 
\end{question}
...
\begin{question}
  Find H, an antiderivative of the function $h$, where $h(x)=5\sec^{2}{x}$.
  \begin{multipleChoice}
  ...
  \begin{feedback}
    Differentiate each choice for $H(x)$.
     ...
     In part (a) we get $H'(x)=5\sec^{2}{x}$.
  \end{feedback}
\end{question}

It is easy to recognize an antiderivative: we just have to differentiate it, and check wether $H'(x)=h(x)$, for all $x$ in the domain of $h$.
But, can we on our own find an antiderivative of a  constant multiple of a function?
\begin{question}
Let $f$ be function and $F$ its antiderivative, i.e., 

$F'(x)=f(x)$ on some interval $I$.

  Find $H$, an antiderivative of the function $h$, where $h(x)=k f(x)$.
  \begin{multipleChoice}
    \choice{$H(x)=k f(x)$}
    \choice[correct]{$H(x)=k F(x)$}
    \choice{$H(x)=F(k x)$}
  \end{multipleChoice}
  \begin{feedback}
    Differentiate each choice for $H(x)$. In part (b) we get $H'(x)=\left(kF(x)\right)'=kF'(x)=k f(x)=h(x)$.
    Therefore, $kF$ is an antiderivative of $kf$.
  \end{feedback}
  So, an antiderivative of a constant multiple of a function is the constant multiple of an antiderivative.
\end{question}
...
We advise that the mathematicians view antiderivatives as a sort of
...
\[
\int \mathrm{something}' \cdot (\mathrm{something})^n \d x
\]
guess
\[
\  \frac{{(something)}^{n+1}}{n+1}
\]
where $n\ne -1$.
\end{template}
