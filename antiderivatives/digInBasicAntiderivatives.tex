...
A function $Ce^x$ is called a \dfn{general solution} of the differential equation. Since there are infinitely many solutions to a differential equation, we can impose additional condition, called an \dfn{initial condition}, e.g. $f(0)=1$. When we are asked to find a function $f$ that satisfies both the differential equation (DE) and the initial condition (IC), this is called an \dfn{initial value problem} (IVP). Let's try one out.
\begin{example}
