...
\begin{image}
  \begin{tikzpicture}
    ...
    \node at (3,7.5) [text width=5cm] {\large
      Here $y=f(x)$ slopes down and is getting \textit{less steep}.
      In this case the curve is \\\dfn{concave up}.};

    \node at (9,7.5) [text width=5cm] {\large
      Here $y=f(x)$ slopes up and is getting \textit{steeper}.
      In this case the curve is \\\dfn{concave up}.};

    \node at (3,1.5) [text width=5cm] {\large
      Here $y=f(x)$ slopes down and is getting \textit{steeper}.
      In this case the curve is \\\textbf{concave down}.};

    \node at (9,1.5) [text width=5cm] {\large
      Here $y=f(x)$ slopes up and is getting less \textit{steep}.
      In this case the curve is \\\textbf{concave down}.};
  \end{tikzpicture}
\end{image}
