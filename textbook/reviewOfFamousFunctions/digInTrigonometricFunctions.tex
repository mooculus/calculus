...
do this becasue we are simply scaling the triangle, and as we see
above, this make absolutely no difference when computing sine and
cosine.

When the hypotuse is $1$, we find that a convienient way to think
about sine and cosine is via the unit circle.
\begin{image}
\begin{tikzpicture}
	\begin{axis}[
...
\]
are called ``arc'' because they litterally take the length of an
...
\]
However, this notation is missleading as $\cos^{-1}(x)$ and
...
For your convience, we have the following definition:
...
\begin{definition}
  ...
  \begin{align*}
    ...
    \arctan(x) = \theta\qquad &\iff\quad\tan(\theta) = x\text{ and } -\frac{\pi}{2}\le \theta\le \frac{\pi}{2}\\
