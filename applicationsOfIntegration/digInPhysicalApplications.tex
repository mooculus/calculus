...
We can and often do approximate physical objects, like wires, as one dimensional.  In this case, the \dfn{linear density} is a measure of its mass per unit length.
...
\paragraph{Step 2: Approximate} The only instance in which we know how to calculate the mass of a 1D object is when the density is constant.  This is a rather poor approximation for the entire rod, but if each slice is very small, it is a fairly good approximation.  
...
\begin{multipleChoice}
\choice{the left half}
\choice[correct]{the right half}
...
\paragraph{Motivating Example:} Suppose that a non-constant force $F(x)$ acts on an object from $x=a$ to $x=b$.  To find how much work does the force do on the object, what should we do?  Let's try the ``Slice, Approximate, Integrate'' procedure.
...
\begin{multipleChoice}
\choice[correct]{is constant, but the force along neighboring pieces may be different.}
\choice{is constant, and the force along all neighboring pieces must be the same as the force on the indicated one.}
\choice{the force along the entire displacement is constant.}
\end{multipleChoice}
...
&= \answer[given]{ 5.88 \times 10^8 \pi} J
\end{align*}
\end{explanation}
