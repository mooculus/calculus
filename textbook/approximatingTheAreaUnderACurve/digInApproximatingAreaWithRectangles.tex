...
\begin{definition}
  When approximating an area with rectangles, the \dfn{grid points}
  are the $x$-coordinates that determine the edges of the rectangles.
\begin{image}
  \begin{tikzpicture}[
      declare function = {f(\x) = pow(\x,2) + 1;}
      ]
	\begin{axis}[
          ...
          plot coordinates {({-1+(\rectnumber -1)*2/8},{f(-1+(\rectnumber -.5)*2/8)}) ({-1+(\rectnumber)*2/8},{f(-1+(\rectnumber -.5)*2/8)})}
          \closedcycle;
          }
          \addplot [very thick,penColor, smooth] {f(x)};
        \end{axis}
  ...
  \begin{tikzpicture}[
	...
	\begin{axis}[
          ...
          \foreach \rectnumber in {1,2,...,8}
                   {
                     \addplot [draw=penColor,fill=fillp]
                     plot coordinates {({-1+(\rectnumber - 1) * 2/8},{f(-1+(\rectnumber-1) * 2/8)} ) ({-1+(\rectnumber) * 2/8},{f(-1+(\rectnumber-1) * 2/8) })} \closedcycle;
                     }
          \addplot [very thick,penColor, smooth] {f(x)};
...
\begin{question}
  What are the grid points for the rectangles above?
  \begin{enumerate}
  \item $x_0$ = 
  \end{enumerate}
\end{question}
  ...
  \begin{tikzpicture}[
    ...
    \begin{axis}[
      ...
      \foreach \rectnumber in {1,2,...,8}
               {
                 \addplot [draw=penColor,fill=fillp]
                 plot coordinates {({-1+(\rectnumber - 1) * 2/8},{f(-1+\rectnumber * 2/8)} ) ({-1+(\rectnumber) * 2/8},{f(-1+\rectnumber * 2/8)})} \closedcycle;
               }
               \addplot [very thick,penColor, smooth] {f(x)};
               %          \addplot [color=penColor,fill=penColor,only marks,mark=*] coordinates{(1,1)};  %% closed hole         
               %          \addplot [color=penColor,fill=penColor,only marks,mark=*] coordinates{(1.25,1.06)};  %% closed hole       
  ...
  \begin{tikzpicture}[
	...
	\begin{axis}[
          ...
          \foreach \rectnumber in {1,2,...,8}
                   {
                     \addplot [draw=penColor,fill=fillp]
                     plot coordinates {({-1+(\rectnumber - 1) * 2/8},{f(-1+(\rectnumber-.5) * 2/8)} ) ({-1+(\rectnumber) * 2/8},{f(-1+(\rectnumber -.5)* 2/8)})} \closedcycle;
                   }
          \addplot [very thick,penColor, smooth] {f(x)};
%          \addplot [color=penColor,fill=penColor,only marks,mark=*] coordinates{(1,1)};  %% closed hole         
%          \addplot [color=penColor,fill=penColor,only marks,mark=*] coordinates{(1.25,1.06)};  %% closed hole       
