...
\begin{problem}
  Consider the line $x+2y=4$. Find a vector normal to the line at the
  point $(2,1)$.
  \begin{prompt}
    \[
...
\begin{problem}
  ...
  \end{tikzpicture}
    \end{image}
    \textbf{Sketch} the vector normal vector you found above on the
    graph above. \textbf{Label} this normal vector $\vec{n}$.
    \begin{prompt}
  ...
  \begin{problem}
    Now \textbf{draw} and \textbf{label} vectors on the graph above to
    show that:
    \[
    \vec{r}=\vec{v} - 2\proj_{\vec{n}}(\vec{v})
    \]
