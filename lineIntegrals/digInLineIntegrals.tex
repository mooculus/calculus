...
\begin{image}
 \begin{tikzpicture}[scale=.9,opacity =1]
    \draw [ultra thick, gray] plot [smooth] coordinates {(0,2) (2,3) (1.5,2) (4,3)};
    \draw [ultra thick, gray,->,opacity=1] plot [smooth] coordinates {(2,2.9) (1.65,2.4)};
    \draw[black,fill=black] (0,2) circle (.5ex);
    \draw[black,fill=black] (4,3) circle (.5ex);
    \node[black,left] at (0,2) {$\vec{a}$};
    \node[black,right] at (4,3) {$\vec{b}$};
    \node[black] at (2,1) {\scalebox{1}{$\displaystyle \int_C \grad F \dotp\d \vec{p} = F(\vec{b}) - F(\vec{a})$}};
 \end{tikzpicture}
\end{image}
