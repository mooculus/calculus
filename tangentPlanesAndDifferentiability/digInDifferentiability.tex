...
\begin{definition}
Let $F: \R^n\to \R$ and let $\vec{x}=\vector{x_1, x_2, \dots, x_n}$
and $\vec{a} = \vector{a_1, a_2, \dots, a_n}$ be vectors in the
interior of the domain of $F$.  We say $F$ is \dfn{differentiable} at
a vector $\vec{a}$ if there exists $L(\vec{x})=F(\vec{0})+\sum_{i=1}^n c_i
