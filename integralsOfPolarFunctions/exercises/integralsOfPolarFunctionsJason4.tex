...
\begin{image}  
  \begin{tikzpicture}  
    \begin{axis}[  
      ...
      \addplot[data cs=polar,blue,domain=0:360,samples=360,smooth, thick] (x,{2*sin(3*x)}) ;
      \addplot[data cs=polar, red, domain=0:360, samples=360, smooth, thick] (x, {1});      
            \end{axis}  
  \end{tikzpicture}  
\end{image} 
  ...
  \begin{tikzpicture}  
    \begin{axis}[  
     ...
     \draw[very thick, orange] (axis cs:0,0) -- (axis cs:2, 1.55) node [pos=.9, above, rotate=40, orange] {$\theta=\theta_{0}$};
        \draw (axis cs:1.24, -1.47  ) node[blue] {$ r=2\cos(3\theta)$};
      \draw (axis cs:-1, -.89 ) node[red] {$r=1$};
            \end{axis}  
  \end{tikzpicture}  
\end{image} 
