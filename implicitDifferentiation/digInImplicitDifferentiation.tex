...
\]
However, this is not always  necessary or even possible to do. Sometimes we choose to or we have to define a function  \textit{implicitely
...
Nevertheless, we are often faced with a problem of computing derivatives of such functions. Therefore, we need a new technique that will enable us to compute derivatives of implicitly defined functions.
...
$y=\sqrt{1-x^2}$ and $y=-\sqrt{1-x^2}$. Therefore, we can say that  any point $(x,y)$ on the curve, where $x\ne-1$, and $x\ne1$, lies on the graph of some function $f$ defined on some open interval $I$.
  Starting with 
\[
x^2 + y^2 = 1
\]
...
\]
On the other hand, $\ddx y^2$ is somewhat different. Here we assume that $y = f(x)$ for some function $f$, defined on some open interval . Hence, by the chain rule
\begin{align*}
\ddx y^2 &= \ddx (f(x))^2 \\ 
&= 2\cdot f(x) \cdot f'(x) \\
&= 2y\dd[y]{x}.
