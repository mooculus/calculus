...
\title[Dig-In:]{Remainders: First Principles}
...
Now, if the series converges, $\sum_{k=1}^\infty a_k  = \lim_{n \to \infty} s_n$ by definition.  This means that the \emph{finite} sum $s_n = \sum_{k=1}^{n} a_k$ should become as close as we want to the value of the infinite series if we choose a large enough value of $n$.  This means that we can approximate the value of $\sum_{k=1}^\infty a_k$ by using terms in the sequence of partial sums!  

\begin{remark}
Note that for a specified value of $n$, we could also compute $s_n$ for a divergent series, but this is inherently not very meaningful since there is no value associated to $\sum_{k=1}^\infty a_k$ if it diverges.
\end{remark}

Now that we have an idea of how to approximate the value of a convergent series, the next important question to ask is: just how good is this estimate?  This is where the idea of a \emph{remainder} comes in.
...
\begin{remark}
Finding the exact value of the series $\sum_{k=1}^{\infty} \frac{1}{k^2}$ is not easy!  In fact, this problem was first posed by the Italian mathematician Pietro Mengoli in 1650 and became known as the Basel problem. The problem remained unsolved until 1734 by the famous mathematician Leonard Euler.  
\end{remark}
...
\begin{explanation}
Note that we are actually given that $s_n = \frac{2n}{n+1}$.  Since $\lim_{n \to \infty} s_n = \answer{2}$, we have that $\sum_{k=1}^{\infty} a_k$ \wordChoice{\choice{diverges}\choice[correct]{converges to $2$}} .

Since the series \wordChoice{\choice{diverges}\choice[correct]{converges}}, we may define a sequence of remainders $\{r_n\}_{n=1}$.  Furthermore, notice that the relationship $\sum_{k=1}^{\infty} a_k = s_n+r_n$ allows us to find an explicit formula for $r_n$.
...
Thus, if $\sum_{k=n_0}^{\infty} a_k$ converges, then we must have that $\lim_{n \to 0} r_n =0$.  Conceptually, this should not be surprising since this means that as we use larger and larger values for $n$, the terms $s_n$ approximate eventually approximate the value of the series more closely since the error will approach $0$. 
...
\section{Summary}
For a convergent series $\sum_{k=1}^{\infty} a_k$, we often cannot determine its exact value, but we can always write
...
\item For a given value of $n$, $r_n = \sum_{k=n+1}^\infty a_k$ is an \emph{infinite} sum and gives the error made when approximating $\sum_{k=1}^{\infty} a_k$ by $s_n$.  Since $\lim_{n \to \infty} r_n = 0$, by using higher and higher values for $n$, we can eventually make the error as close as we want to $0$.
...
\choice{all the time, even when the series diverges.}
\choice[correct]{when the series converges.}
\choice{only when we have an explicit formula for the terms in the sequence of partial sums.}
\end{multipleChoice}
\end{question}
...
\begin{selectAll}
\choice[correct]{If $\sum_{k=1}^{\infty} a_k$ converges, then $\lim_{n \to \infty} r_n = 0$.}
\choice{It is possible to find a divergent series $\sum_{k=1}^{\infty} a_k$ for which $\lim_{n \to \infty} r_n =0$.}
\choice{It is possible to find a convergent series $\sum_{k=1}^{\infty} a_k$ for which $\lim_{n \to \infty} r_n =3$.}
\choice[correct]{Once we have established that a series $\sum_{k=1}^{\infty} a_k$ converges, we can approximate its value by calculating the \emph{finite} sum $\sum_{k=1}^{n} a_k$.}
\choice[correct]{Once we have established that a series $\sum_{k=1}^{\infty} a_k$ converges, we can think of $r_N$ as the error made when approximating the infinite series $\sum_{k=1}^{\infty} a_k$ by $s_N$.}
\end{selectAll}
\end{question}
