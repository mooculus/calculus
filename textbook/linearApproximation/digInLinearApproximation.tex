...
\begin{tikzpicture}
	\begin{axis}[
	  ...
	  \addplot [draw=black,dashed,->,>=stealth'] plot coordinates {(1.7,10/6) (1.7,10/6 +.3/.36)};
          ...
          \node at (axis cs:1.7,2.08) [right] {$dy$};
        \end{axis}
\end{tikzpicture}
...
\begin{image}
  \begin{tikzpicture}
    \node at (0,0) {
      $\underbrace{f(x + dx)} \approx  \overbrace{f(x)} + \underbrace{dy}$
    };
...
\begin{explanation}
  ...
  $4^3 = 64$, we set $x=64$.  Setting $dx=-14$, we have
  \begin{align*}
  \sqrt[3]{50} = f(x + dx) &\approx  f(x) + dy\\
  &\approx \sqrt[3]{64} + dy.
  \end{align*}
  ...
  \begin{image}
    \begin{tikzpicture}
      \begin{axis}[
        ...
        \node [left] at (axis cs:50,3.86) {$dy$};
...
\begin{explanation}
Set $f(x) = \sin(x)$. We want to know $\sin(0.3)$. Since $\sin(0) =
0$, we will set $x = 0$ and $dx=0.3$. Write with me
\begin{align*}
  \sin(0.3) = f(x + dx) &\approx  f(x) + dy\\
  &\approx 0 + dy.
  \end{align*}
...
\begin{align*}
  ...
  &= 0.3
\end{align*}
Hence $f(0.3) \approx f(0) + 0.3 \approx 0.3$.
\end{explanation}
\end{example}
...
%%   As a gesture of friendship, we will tell you that the volume is given by
%%   \[
%%   V(x) = \pi r(x)^2.
%%   \]
%%   \begin{explanation}
%%     This question is ripe to be solved with differentials. We are
%%     looking for $dV$, but we know
%%     \[
%%     dV = V'(x) \d x
%%     \]
%%     Since
