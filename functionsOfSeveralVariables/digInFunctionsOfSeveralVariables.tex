...
each point along the curve $\vec{p}$ and produce another curve $F\left (\vec{p} \right )$
on the surface.
...
So far, we have focused mainly on the curve $\vec{p}(t)= \vector{x(t), y(t)}$ lying in the 
    domain of a function $F$.  Let's now focus on the curve $ \vector{x(t), y(t), F \left ( \vec{p} (t) \right )}$ 
...
\begin{example}
  ...
  description of the the curve $\vec{r}(t)$ that lies on the surface $z=F(x,y)$ above the line
...
curve on the graph of $F$ can be helpful when thinking about many topics 
...
we are working in the domain of $F$ or on the surface $z=F(x,y)$ itself.
