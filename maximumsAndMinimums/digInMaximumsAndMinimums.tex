...
\begin{definition}\hfil\index{maximum/minimum!global}
	\begin{enumerate}
		\item A function $f$ has an \dfn{global maximum} at $x=a$, 
			if $f(a) \geq f(x)$ for every $x$ in the domain of the function.
		\item A function $f$ has an \dfn{global minimum} at $x=a$, 
			if $f(a) \leq f(x)$ for every $x$ in the domain of the function.
	\end{enumerate} 
	A \dfn{global extremum}\index{extremum!global} is either a global maximum or a global minimum.
	These are occasionally referred to as \dfn{absolute extrema} as well.
\end{definition}
		...
		\begin{axis}[
		  ...
		  width=6in, height=4in,
	            ...
	            every axis x label/.style={at=(current axis.right of origin),anchor=west}
		          ...
		          \node at (axis cs:2,4) [penColor2, below right] {$y=x^2$};
	        \end{axis}
	     
		\end{tikzpicture}
		%% \caption{A plot of $f(x) = x^2$.}
		%% \label{figure:x^2}
		\end{image}
		There is no highest point on this graph since $\displaystyle \lim_{x\to \pm \infty}x^2 = \infty$, but there is a lowest point.
	...
	\begin{explanation}
		...
		\begin{tikzpicture}
			\begin{axis}[
		          ...
		          \node at (axis cs:-1.57,.75) [penColor2, above left] {$y=\cos(x)$};
		        \end{axis}
		\end{tikzpicture}
		\end{image}
...
\begin{definition}\hfil\index{maximum/minimum!local}
	\begin{enumerate}
		\item A function $f$ has a \dfn{local maximum} at $x=a$, if $f(a)\geq
		  	f(x)$ for every $x$ in some open interval I containing $a$.
		\item A function $f$ has a \dfn{local minimum} at $x=a$, if $f(a)\leq
		  	f(x)$ for every $x$  in some open interval I containing $a$.
	\end{enumerate}
	A \dfn{local extremum}\index{extremum!local} is either a local
	maximum or a local minimum. These are sometimes referred to as \dfn{relative extrema} as well.
\end{definition}
