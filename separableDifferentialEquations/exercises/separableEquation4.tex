...
For $b=\frac{1}{2}$, it looks like $\lim_{x\to -2^+} = \answer{\infty} y(x)$ from the direction field.

For $b=2$, it looks like $\lim_{x\to -2^+} = \answer{\infty} y(x)$ from the direction field.

For any $b>0$, does is appear that $\lim_{x\to -2^+} = \answer{\infty} y(x)$ depends on $b$?
...
\begin{exercise}
To verify the conjecture above, we need to solve theinitial value problem.
