...
\begin{tikzpicture}
  \begin{axis}[
          ...
          \node at (axis cs:6.666,2.334) [anchor=north] {$x_2$};
          \node at (axis cs:5.334,2.334) [anchor=north] {$x_1$};

          \node at (axis cs:5.334,3) [anchor=east] {$f(a)$};
          \node at (axis cs:5.334,3.666) [anchor=east] {$f(x_2)$};
          \node at (axis cs:5.334,2.334) [anchor=east] {$f(x_1)$};
...
\begin{problem}
  ...
  \begin{selectAll}
  	\choice{$\frac{f(x_1)-f(a)}{x_2-a}$}
	\choice{$\frac{f(x_1)-f(a)}{a-x_1}$}
  	\choice[correct]{$\frac{f(x_1)-f(a)}{x_1-a}$}
	\choice{$\frac{f(x_1)-a}{x_1-a}$}
	\choice[correct]{$\frac{f(a)-f(x_2)}{a-x_2}$}
	\choice[correct]{$\frac{f(x_2)-f(a)}{x_2-a}$}
	\choice{$\frac{x_2-a}{f(x_2)-f(a)}$}
	\choice{$\frac{f(x_1)-f(a)}{f(x_2)-f(a)}$}
	\choice{$a(f(x_2)-f(x_1))$}
	\choice[correct]{$\frac{f(a)-f(x_1)}{a-x_1}$}
  \end{selectAll}
\end{problem}

\begin{problem}
   Let $f(x) = 3x-1$.  Zoom in on the curve around $a = -2$ so that $x_1
   = -1.9$.  Use one of the formulations in the problem above to
   approximate the slope of the curve.  The slope of the curve at $a =
   -2$ is approximately\dots \begin{prompt}$\answer{3}$\end{prompt}
\end{problem}

\begin{problem}
   Repeat the previous problem for $f(x) = x^2 - 1$, $a = 0$, and $x_2 =
