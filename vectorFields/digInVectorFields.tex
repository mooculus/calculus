...
f:\R \to \R
\]
Typically, we interpret these functions as being curves in the
$(x,y)$-plane:
\begin{image}
\begin{tikzpicture}
	\begin{axis}[
...
\vec{f}:\R \to \R^n
\]
We can interpret these functions as parametric curves in space:
\begin{image}
    \begin{tikzpicture}
      \begin{axis}%
...
F:\R^n \to \R
\]
We can interpret these functions as surfaces in $\R^{n+1}$,
\begin{image}
  \begin{tikzpicture}
    \begin{axis}%
...
\vec{F}:\R^n\to \R^n
\]
and we interpret these as \textit{vector fields}, meaning for each
