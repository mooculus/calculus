...
fact. You may be uncomfortable because you can't quite see why we need
this, but this style of exposition is a fact of technical writing; it
...
\]
from this result we see another example of a limit:
\begin{example}
  \[
...
After these delicious aptizers, we are now ready for the main course.

\begin{theorem}[The Derivative of sin($\theta$)]\index{derivative!of sine}\label{theorem:deriv sin}
\[
\dd{\theta} \sin(\theta) = \cos(\theta).
\]
\begin{proof}
...
&= \lim_{h\to0} \frac{\sin(\theta)\cos(h)+\sin(h)\cos(\theta)-\sin(\theta)}{h}  & \text{Trig Identity.}\\
...
&=\sin(\theta) \cdot \answer[given]{0} + \cos(\theta) \cdot \answer[given]{1} = \cos \theta. 
\end{align*}
\end{proof}
\end{theorem}
...
Pro tip: When working with trigonometric functions, you must always
remember their graphical representations.
