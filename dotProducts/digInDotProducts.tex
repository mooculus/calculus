...
\begin{definition}
  ...
  Note: Geometrically, this means that the angle
  between two nonzero vectors is $\pi/2$ or $90^\circ$. This also means
...
\begin{question}
  Find all unit vectors orthogonal to both vectors $\vec{v}$ and  $ \vec{w}$, given by
  \begin{align*}
    \vec{v} &= \veci + \vecj\\
    \vec{w} &= \veci + \veck .
  \end{align*}
