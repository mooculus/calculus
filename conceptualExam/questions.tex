...
\begin{problem}
  \recommendation{Vic}
\textbf{True} or \textbf{False}. At some time since you were
...
\begin{problem}
  \recommendation{Vic} \textbf{True} or \textbf{False}.  If
...
\begin{problem}
  ...
  \begin{tikzpicture}
	\begin{axis}[
            ...
            axis on top = true,
            xtick={-7}, ytick={3,6},
            xticklabels={$a$}%, yticklabels={$f(x)$,$f(x+h)$}
          ]        
          \addplot [very thick, penColor, samples=150,smooth,domain=(-9:2)] {7+8*x + (97*x^2) / 49 + (48*x^3) / 343};
          \addplot [very thick, penColor3, samples=150,smooth,domain=(-9:2)] {7 + (9*x) / 2 + (48*x^2) / 49 + (47*x^3) / 686};
          ...
          \node at (axis cs:-1.8,1) [penColor2] {$g$};
          \node at (axis cs:1,6) [penColor4] {$L_1$};
          \node at (axis cs:1,2.8) [penColor3] {$L_2$};
        \end{axis}
  \end{tikzpicture}
  \end{image}
...
\begin{problem}
  ...
  does $\int_{18}^{19} W'(t) \, dt = 15$ mean?
  \begin{multipleChoice}
