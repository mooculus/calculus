...
\begin{align*}
\int u \d v &= uv - \int u \d u & \textrm{(integration by parts for indefinite integrals)} \\
\int_a^b u \d v &= \eval{uv}_a^b - \int_a^b u \d u & \textrm{(integration by parts for definite integrals)}
\end{align*}
\end{formula}
