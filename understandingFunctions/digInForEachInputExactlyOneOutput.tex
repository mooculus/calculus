...
numbers, which is denoted in interval notation by $(-\infty, \infty)$. In our next examples we show that this is
not always the case.
(Occasionally you will see an alternative notation for the set of all real numbers, $\RR$. For our purposes, sets like the domain and range should be given in interval notation.)
...
f(x) = \sqrt{x}.
\]
The domain is $0\le x<\infty$, which we write in interval notation as
$[0,\infty)$. The range is $[0,\infty)$.  Here is
    a graph of $y=f(x)$:
\begin{image}
\begin{tikzpicture}
	\begin{axis}[
...
have the same domain. They cannot be the same function.
