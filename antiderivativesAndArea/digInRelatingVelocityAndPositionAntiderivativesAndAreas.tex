...
\begin{document}
\begin{abstract}
We see a connection between  antiderivatives and areas. 
\end{abstract}
\maketitle
...
\[
     displacement=velocity\cdot (elapsed\hspace{0.05in} time),
      \]  
         ...
         The graph of the (constant) velocity function $v$ on the interval $[2,3]$ is given in the figure.
      \begin{image}
  \begin{tikzpicture}[
...
\end{image}
      ...
      So, the displacement of the object is given by the product of the  velocity and elapsed time, and is represented by the area of the shaded rectangle in the figure. 
   \end{feedback}
  \end{question}
...
Assume an object is moving along a straight line  with the velocity $v(t)=\frac{t^2}{4}+1$, for $0\le t\le4$,where $t$ is measured in seconds, and $v$ in $\frac{ft}{s}$.
  ...
  \begin{explanation}
   Since we know how to compute the displacement when the velocity is constant, let's assume, first, that the velocity is constant and equal to $v(0)$, the initial velocity, for the entire  four-second time interval, $\Delta t=4$. 
...
If we $\Delta t$ denote the length of the time interval, we can approximate the displacement  and write
...
\end{tikzpicture}
\end{image}
Now, we approximate the displacement of the object assuming that the velocity was constant during half-second time intervals, and that velocity is equal to the velocity at the beginning of each time interval.
...
We continue by approximating the displacement of the object assuming that the velocity is constant during $4/n$-second time intervals, with velocity equal to the velocity at the beginning of each time interval.
 ...
 Under that assumption, during the first $\frac{4}{n}$-second interval, the object would move by $v(0)\cdot\frac{4}{n}$;  during the second such time interval, the object would move by $v\left(\frac{4}{n}\right)\cdot\frac{4}{n}$, during the third time interval, the object would move by $v\left(2\frac{4}{n}\right)\cdot\frac{4}{n}$,etc. 
...
Is it true that for any function $f$ and its antiderivative $F$, we can compute the definite integral of $f$ on the interval $[a,b]$ by this simple formula
\[
