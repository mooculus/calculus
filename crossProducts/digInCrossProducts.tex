...
\begin{definition}
  Given a $3\times 3$ matrix, the \dfn{determinant} is denoted by:
  \[
  \det\begin{bmatrix}
  a_1 &  a_2 & a_3 \\
  b_1 &  b_2 & b_3 \\
  ...
  \end{vmatrix}
  \]
  It can be computed using the following pattern:
\begin{image}[3in]
\begin{tikzpicture}
\node at (0,0) {$\det
\begin{bmatrix}
...
Now let's examine the cross product on famous unit vectors.
\begin{question}
  Compute:
  \[
  \veci \cross \vecj
...
Note, if $\vec{a}$ and $\vec{b}$ are in $\R^2$, we can \textit{still}
use the cross product to compute the area of the parallelogram spanned
by $\vec{a}$ and $\vec{b}$, just add a $z$-component of $0$ to each
vector.
...
\begin{theorem}
  ...
  \]
  \begin{explanation}
    This follows since the area of the parallelogram spanned by
