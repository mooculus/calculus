...
\begin{tikzpicture}
	\begin{axis}[
	  ...
	  \addplot [draw=black,dashed,->,>=stealth'] plo,t coordinates {(1.7,10/6) (1.7,10/6 +.3/.36)};
...
\item $\d y$ is the change in $y$, where $y=L(\d x)$  and, therefore,
  \[
  \d y = f'(x)\d x
  \]
  Note: $ L(\d x)= f(x)+f'(x)\d x$.
   
  So, the change
   \[
  \d y = L(\d x)-L(0)= f(x)+f'(x)\d x-L(0)= f(x)+f'(x)\d x-f(x)=f'(x)\d x
  \]
\end{itemize}
\begin{question}
