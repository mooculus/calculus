...
There is a somewhat wide variation on what graphs of rational
functions look like.
...
Match the curves $A$, $B$, $C$, and $D$ with the functions
  \[
  \frac{x^2-3x+2}{x-2}, \qquad \frac{x^2-3x+2}{x+2}, \qquad \frac{x-2}{x^2-3x+2}, \qquad \frac{x+2}{x^2-3x+2}.
  \]
\begin{explanation}
  Consider $\frac{x^2-3x+2}{x-2}$. This function is undefined only at
  $x=2$. Of the curves that we see above, only one is undefined at a
  single point, curve $\answer[given]{D}$.
  ...
  $B$ and $D$ would work. Distinguishing between these two curves is
