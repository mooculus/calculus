...
Let $f(x)=x\ln x$. 
\begin{exercise}
The domain of $f$ is $\answer{(0,\infty)}$.
\begin{exercise}
...
\begin{exercise}
Since both $\ln x$ and $1/x$ are differentiable functions on an interval of the form $(0,\varepsilon)$ where $\varepsilon>0$, we may use L'H\^{o}pital's rule to compute
\[
\lim_{x\to 0^+}f(x)=\answer{0}
\]
\begin{exercise}
