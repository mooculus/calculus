...
\end{exercise}
\begin{exercise}
Suppose that the function $s$ is continuous on $[0,10)$ and differentiable on $(0,10)$. Then  on the interval $[0,4]$ the function $s$ \wordChoice{\choice[correct]{satisfies}\choice{does not satisfy}} the conditions of the Mean Value Theorem so \wordChoice{\choice[correct]{there exists}\choice{there does not necessarily exist}} a number c in the interval $(0,4)$ such that $s'(c)$ equals the above calculated average velocity.
