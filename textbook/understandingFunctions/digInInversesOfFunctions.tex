...
\begin{warning}
  We define $f(x) = \sqrt{x}$ to be the positive square-root, so that we can be sure
   that $f$ is a function. Thinking of the square-root as the inverse of the squaring function, 
   we can see the issue a little more clearly.  There are two $x$-values that square to $9$.
  \[
  x^2 = 9\qquad\text{means $x=\pm 3$}
  \]
  Since we require that \textbf{square-root is a function}, we must have only one output value
  when we plug in $9$.  We choose the positive square-root, meaning that
  \[
  \sqrt{9}  =3.
  \]
\end{warning}
...
interval $[0,\infty)$. Hence we can find an inverse of $f(x)=x^2$ on
  ...
  \end{align*}
  Since the domain of $f$ is $[0, \infty)$, we know that $x$ is positive.  This means we can 
  take the square-root of each side of the equation to find that
    \[
    \sqrt{x} = f^{-1}(x).
    \]
    \begin{image}
\begin{tikzpicture}
	\begin{axis}[
	  ...
	  \addplot [very thick, penColor, smooth] {x^2};
          ...
          \node at (axis cs:-1,.55) [penColor] {$f(x)$};
          \node at (axis cs:1.4,.8) [penColor2] {$f^{-1}(x)$};
        \end{axis}
\end{tikzpicture}
...
\begin{tikzpicture}
	\begin{axis}[
	  ...
	  \addplot [very thick, penColor, smooth] {x^3};
          ...
          \addplot [dashed, textColor] {x};
          \node at (axis cs:-.8,-1.5) [penColor] {$f(x)$};
          \node at (axis cs:1.5,.9) [penColor2] {$f^{-1}(x)$};
        \end{axis}
\end{tikzpicture}
