...
\end{image}
\begin{example}
Here we clarify the definition of a local minimum.\\

In the figure above the function $f$ has a local minimum at ,
\[
a=\answer[given]{1},
\]
 because were able to find  an \textbf{open} interval $I$ (marked in the figure) that contains the number
 \[
a=\answer[given]{1},
\]
 and for all numbers $x$ in $I$ the following statement is true:
 \[
f(\answer[given]{x})\le f(\answer[given]{1}).
\] 
\end{example}
...
$x=1$. Since the derivative, $f'(x)$, is a polynomial, it does not change the sign on intervals between its zeros, i.e., between its the critical points. Now we can check the sign of $f'(x)$ at some points \textbf{between} the critical points to find
