...
\choice{If $s_n = n$, then it is possible that $\lim_{n \to \infty} r_n =0$}.
...
\item If $s_n=n$, then $\sum_{k=1}^{\infty} a_k$ \wordChoice{\choice{converges}\choice[correct]{diverges}}.  Since $\sum_{k=1}^{\infty} a_k$ converges if and only if $\lim_{n \to \infty} r_n = \answer{0}$, we see that statement  (c) is \wordChoice{\choice{true}\choice[correct]{false}}.
\end{itemize}
\end{hint}
\end{exercise}
\end{document}