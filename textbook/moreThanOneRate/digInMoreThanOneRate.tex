...
\begin{example}
  ...
  \end{image}
    ...
    2\cdot h(t).
    \]
    We are now able to \textbf{differentiate the equation(s)} using
    implicit differentiation, treating all functions as functions of
    $t$,
    \[
    A'(t) = (1/2) \cdot x'(t) \cdot h(t) +  (1/2) \cdot x(t) \cdot h'(t)\qquad\text{and}\qquad x'(t) = 2\cdot h'(t)
    \]
    Now we \textbf{evaluate the equation(s) at the desired values.} We
    know that $x'(t) = 3$ and that $x(t) = 5$. Since
    \[
    x(t) = 2\cdot h(t),
    \]
    we see that $h(t) = 3/2$. Hence
    \begin{align*}
