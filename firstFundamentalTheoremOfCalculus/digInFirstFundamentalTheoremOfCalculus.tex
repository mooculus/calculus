...
\end{example}
\begin {example}
Given the graph of $f$ on the interval $[-1,5]$, sketch the graph of accumulation function 
 \[
...
\begin{tikzpicture}
  \begin{axis}[
    ...
    \addplot [penColor,very thick,domain=-1:3] {x-1 };
      \addplot [penColor,very thick,domain=3:5] {2 };
...
\]
We plot these six points in the figure below.
\begin{image}
\begin{tikzpicture}
  \begin{axis}[
      xmin=-1.5, xmax=5.2,ymin=-2.2,ymax=4.2,domain=-1:5.5,
      axis lines =center, xlabel=$t$, ylabel=$y$,
      every axis y label/.style={at=(current axis.above origin),anchor=south},
      every axis x label/.style={at=(current axis.right of origin),anchor=west},
      xtick={-1,0,1,2,2.6,3,4,5}, 
...
\]
Since the function $f$, the derivative of $F$, is increasing on $(1,3)$, and  it follows that $F$ is concave up on the interval
\[
\left(\answer[given]{1},\answer[given]{3}\right).
\]
We can conclude now that the function $F$ has  a local and global minimum at
\[
x=\answer[given]{1},
\]
  ...
  \begin{axis}[
    ...
    \addplot [penColor,very thick,domain=-1:3] {x-1 };
     ...
     \addplot[color=penColor2,fill=penColor,only marks,mark=*] coordinates{(-1,0)};  %% closed hole
        \addplot[color=penColor2,fill=penColor,only marks,mark=*] coordinates{(0,-1.5)};  %% closed hole
      \addplot[color=penColor2,fill=penColor,only marks,mark=*] coordinates{(1,-2)};  %% closed hole
       \addplot[color=penColor2,fill=penColor,only marks,mark=*] coordinates{(2.6,-0.72)};  %% closed hole
      \addplot[color=penColor2,fill=penColor,only marks,mark=*] coordinates{(3,0)};  %% closed hole
        \addplot[color=penColor2,fill=penColor,only marks,mark=*] coordinates{(4,2)};  %% closed hole
      \addplot[color=penColor2,fill=penColor,only marks,mark=*] coordinates{(5,4)};  %% closed hole
      \addplot [penColor2,very thick,domain=-1:3] {(1/2)*x^2-x-3/2 };
      \addplot [penColor2,very thick,domain=3:5] {2*x-6};
    \node at (axis cs:2.2,.65) [textColor] {\scalebox{2}{$\boldsymbol+$}};
    \node at (axis cs:-.55,-.6) [textColor] {\scalebox{2}{$\boldsymbol-$}};
      \node at (axis cs:2.3,2) [penColor] {\huge$f$};
         \node at (axis cs:3.8,-1.45) [penColor2] {\huge$F(x)= \int_{-1}^x f(t) \d t$};
  \end{axis}
\end{tikzpicture}
%\caption{The integral $\int_{-1}^x t^3 \d t$ measures the shaded area.}
%\label{figure:accumulationeg}
...
What happens when the function $f$ is the velocity of an object moving along a straight line, i.e. $f(t)=v(t)$, $a\le t\le b$.

What is the meaning of accumulation function in that case?


We use different variables, since we want $F$ to be a function of time, $t$. With that adjustment, we define an accumulation function $F$ as follows.
\[
  F(t)=\int_a^t v(z)\d z.
\]
Since the integral
