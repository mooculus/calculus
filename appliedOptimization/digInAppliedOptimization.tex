...
%\draw[very thick,penColor2] (-2,0) arc (180:360:2 and .7);% bottom half of ellipse
\draw [penColor2, very thick] (0,0) circle [radius=2];
\draw[penColor2, dashed, very thick] (0,0) --(1.7,1);
\draw[penColor2, dashed] (0,0) --(0,-2);
\draw[penColor, dashed, very thick] (0,1) -- (1.7,1);
\draw[penColor5, dashed, very thick] (0,0) -- (0,1);
\node [below,penColor2] at (0.8,0.53) {$R$};
\node [below,penColor2] at (0.2,-0.6) {$R$};
\node [above,penColor] at (.85,1) {$r$};
\node [left,penColor] at (0,-.33) {$h$};
\node [left,penColor5] at (0,.53){$h-R$};
...
\] 
we find that the function has critical points at $h=0$ and $h=4R/3$.  We evaluate the function at all the critical and end points
\[
