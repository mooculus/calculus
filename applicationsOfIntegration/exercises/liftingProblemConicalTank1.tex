...
An inverted conical tank has base with diameter 6m and height 9m. Suppose the tank is filled to a height of 6m with acetone ($\rho$=785 kg/m$^3$). Find the work required to pump the acetone out of the tank (use $g=9.8$m/s$^2$).
\begin{image}
\begin{tikzpicture}
\begin{axis}[
domain=-3:3,
...
\node[anchor=west] at (axis cs:2.2,3) {$6$m};
...
\[
W= \int_{y=\answer{0}}^{y=\answer{6}} \answer{7693\pi\left(\frac{y}{3}\right)^2 (9-y)}\d y = \answer[tolerance=10]{276948\pi} J
\]
...
To move a slice to a height $y=10$, we find the distance $d(y)$ the slice at height $y$ must be moved is $d(y) = \answer{9-y}$
