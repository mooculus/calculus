...
\end{tikzpicture}
\end{image}
    At $t=2$, $s'(2) = v(2) = 6$, so we attach a segment of slope $6$ over an additional interval of length $2$:
    \begin{image}
  \begin{tikzpicture}[
      declare function = {f(\x) = 6+ \x/2 - pow(\x,2)/4;} ]
	\begin{axis}[
...
\end{tikzpicture}
    \end{image}
   At $t=6$, $s'(6) = v(6) = 0$, so we attach a segment of slope $0$ over an additional interval of length $2$:
    \begin{image}
  \begin{tikzpicture}[
      declare function = {f(\x) = 6+\x/2 - pow(\x,2)/4;} ]
	\begin{axis}[
...
\end{tikzpicture}
\end{image}
    At $t=8$, $s'(8) = v(8) = -6$, so we attach a segment of slope $-6$ over an interval of length $2$:
    \begin{image}
  \begin{tikzpicture}[
      declare function = {f(\x) = 6+ \x/2 - pow(\x,2)/4;} ]
	\begin{axis}[
