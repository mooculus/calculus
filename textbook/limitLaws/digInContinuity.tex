...
\item $f(x)=x^k$, $-\infty<x<\infty$ (Note: When $k$ is even, we
  insist $0< x < \infty$.) %% at zero is a problem
...
This theorem can be summarized by saying that the functions listed above are continuous wherever they are defined, that is, on their natural domain. For many of the functions we are most familiar with, we can use
