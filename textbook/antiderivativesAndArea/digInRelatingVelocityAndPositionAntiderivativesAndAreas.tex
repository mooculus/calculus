...
\begin{question}
   Suppose you in slow traffic moving at $4$ \textrm{mph} from $2$pm
   to $5$pm.  How far have you traveled?
   \begin{prompt}
     \[
     \answer{12}\ \text{miles}.
     \]
...
\begin{center}
  ``What is the area of a rectangle with width $3$ and height $4$?''
\end{center}
%\end{idea}
...
\begin{example}
  ...
  \begin{explanation}
    ...
    $t=0$, $s'(0) = v(0) = 6$, so we start with:
    \begin{image}
  \begin{tikzpicture}[
	...
	\begin{axis}[
          ...
          %\addplot [draw=penColor,ultra thick] plot coordinates {(0,0) (2,0) (4, {f(2)*2}) (6, {f(2)*2+f(6)*2}) (8, {f(2)*2+f(4)*2 + f(6)*2}) (10,{f(2)*2+f(4)*2 + f(6)*2 +f(8)*2} )};
        \end{axis}
\end{tikzpicture}
\end{image}
    At $t=2$, $s'(2) = v(2) = 6$, so we attach a segment of slope $4$ over an additional interval of length $2$:
    \begin{image}
  \begin{tikzpicture}[
      declare function = {f(\x) = 6+ \x/2 - pow(\x,2)/4;} ]
	\begin{axis}[
...
\end{tikzpicture}
\end{image}
    At $t=4$, $s'(4) = v(4) = 4$, so we attach a segment of slope $4$ over an additional interval of length $2$:
    \begin{image}
  \begin{tikzpicture}[
      declare function = {f(\x) = 6+\x/2 - pow(\x,2)/4;} ]
	\begin{axis}[
...
\end{tikzpicture}
    \end{image}
   At $t=6$, $s'(6) = v(6) = 4$, so we attach a segment of slope $4$ over an additional interval of length $2$:
    \begin{image}
  \begin{tikzpicture}[
      declare function = {f(\x) = 6+\x/2 - pow(\x,2)/4;} ]
	\begin{axis}[
...
\end{tikzpicture}
\end{image}
    At $t=8$, $s'(8) = v(8) = 0$, so we attach a segment of slope $0$ over a final interval of length $2$:
    \begin{image}
  \begin{tikzpicture}[
      declare function = {f(\x) = 6+ \x/2 - pow(\x,2)/4;} ]
	\begin{axis}[
...
\begin{example}
  ...
  \begin{explanation}
	...
	\begin{axis}[
          ...
          \node at (axis cs:4,40) {\Large${\color{penColor}v(0)\cdot 2} + {\color{penColor2}v(2)\cdot 2} + {\color{penColor3}v(4) \cdot 2} + {\color{penColor4}v(6)\cdot 2} + {\color{penColor5}v(8)\cdot 2}$};
        \end{axis}
\end{tikzpicture}
    \end{image}
    So
...
\d s = v(t)\d t
\]
and adding it to the previous result.  If we wanted to, we could take
