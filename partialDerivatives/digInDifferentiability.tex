...
We would like to generalize this notion of a tangent line to functions of several variables.  We would like to say $F$  is differentiable when one can ``zoom-in'' and the surface looks "flat".  

Let's start by considering functions from $\R^2\to \R$ as these are the easiest to visualize. In this case, if our function is differentiable, when we "zoom-in", $F: \R^2\to \R$ should look like a plane. We call this plane the \dfn{tangent plane}. 
...
We can interpert this definition as saying that a function $F$ is differentiable at a vector$\vec{a}$ if there is a tangent plane $T_a(\vec{x})$ at that point such that $F$ approaches its  tangent plane $T_a(\vec{x})$ faster than $\vec{x}$ approaches $\vec{a}$.  That is, if one ``zooms in'' on the graph at $(\vec{a}, F(\vec{a}))$ sufficiently, $F$  looks just like the tangent plane.
...
Keep in mind that this statement of differentiability is much stronger than just saying the partial derivatives of $F$ exist at $\vec{a}$.  Because this is a limit in a higher dimension, we are saying this limit must exist for every path by which $\vec{x}$ could approach $\vec{a}$, not just the paths along the axes.  Because of this, it is very difficult to use this definition to show that a function is differentiable.  Thankfully we have the following theorem:
...
the following criteria about our tangent plane:
\[
F(\vec{a}) =T_a(\vec{a})
\]
\[
