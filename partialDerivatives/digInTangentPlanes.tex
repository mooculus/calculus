...
point.  We have called this plane the \dfn{tangent plane} to $F$ at
    ...
    \begin{image}
      \begin{tikzpicture}
        \begin{axis}[
          ...
          \addplot[decoration={brace,raise=.2cm},decorate,thin] plot coordinates {(1,.1) (1,.9)};
          \node[left] at (axis cs: .9,.5) {$f(a)$};
        \end{axis}
\end{tikzpicture}
    \end{image}
    So we see that:
