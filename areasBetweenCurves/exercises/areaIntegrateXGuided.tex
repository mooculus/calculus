...
\begin{exercise}
Since we are computing the area as an integral with respect to \wordChoice{\choice[correct]{$x$} \choice{$y$}} , we must use \wordChoice{\choice[correct]{vertical} \choice{horizontal}} slices:
\end{exercise}
...
We must now express $h$ in terms of the variable of integration.  Since $h$ is a vertical distance, $h=$ \wordChoice{\choice[correct]{$y_{top}-y_{bot}$} \choice{$x_{right}-x_{left}$}}.
\end{exercise}
