...
Process of approximating areas under curves led to the notion of a Riemann sum
...
What happens when a continuous function $f$ on an interval $[a,b]$ assumes both negative and positive values on $[a,b]$?
...
The limit will exist for any continuous functions on the interval $[a,b]$, even if $f$ assumes negative values on $[a,b]$.
...
\]
\begin{prompt}
The  area between the $x-$axis and the curve can be easily computed, since it is the area of the triangle
\[
...
\]
On the other hand, the function $f(x)=-x$ is very simple, so we can compute the definite integral using the \textbf{definition}.
\[
\int_0^{10}-x \d x=\lim_{n\to\infty}\sum_{k=1}^n f(x_k^*)\Delta x=\lim_{n\to\infty}\sum_{k=1}^n -x_k^*\Delta x
\]
It does not matter what  type of a Riemann sum we use, so we can choose a right Riemann sum.
...
We can apply some rules for sums and limits and obtain
...
\]
The last expression is clearly the signed area of the region between the x-axis and the curve $y=-x$. We already used that fact.

Now we want to compute the limit from scratch.
\begin{align*}
\int_0^{10}-x \d x&=-\lim_{n\to\infty}\sum_{k=1}^n x_k\Delta x\\
